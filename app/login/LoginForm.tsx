...
export default function LoginForm({ 
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden px-3 py-4 md:px-0 md:py-0" style={{ 
      ...backgroundStyle,
      marginTop: 0, 
      paddingTop: 0 
    }}>
      {/* Overlay nhẹ để đảm bảo form dễ đọc - tối hơn trên mobile */}
      {backgroundUrl && (
        <div className="absolute inset-0 bg-black md:bg-opacity-10 bg-opacity-20"></div>
      )}
      ...
      {/* Logo VNPT at top with glow effect - chỉ hiển thị khi không có ảnh nền, ẩn hoàn toàn trên mobile để tiết kiệm không gian */}
      {!backgroundUrl && (
      ...
      <div 
        ...
        style={formPosition && !isMobile ? {
          ...
          maxWidth: formPosition ? '95%' : '20rem',
          width: '100%',
        }}
      >
        <div className="bg-white rounded-lg md:rounded-xl shadow-lg md:shadow-2xl backdrop-blur-sm w-full p-4 md:p-6 border border-gray-100 md:border-gray-200">
          <div className="text-center mb-3 md:mb-8">
            <h1 className="text-lg md:text-3xl font-bold text-gray-900 mb-1 md:mb-2">Đăng nhập</h1>
            <p className="text-xs md:text-base text-gray-600 leading-tight md:leading-normal">{loginSubtitle}</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-2.5 md:space-y-6">
            <div>
              <label className="block mb-1 md:mb-2 text-xs md:text-base font-semibold text-gray-700">Tên đăng nhập</label>
              <input
                type="text"
                value={username}
            ...
            <div>
              <label className="block mb-1 md:mb-2 text-xs md:text-base font-semibold text-gray-700">Mật khẩu</label>
              <div className="relative">
                <input
                  type={showPassword ? 'text' : 'password'}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="input-field pr-9 md:pr-10 text-base md:text-base"
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-700 px-2.5 md:px-4 py-1.5 md:py-3 rounded text-xs md:text-base">
                {error}
              </div>
            )}
            ...
            <div className="flex items-center pt-0.5">
              <input
                ...
                className="w-3.5 h-3.5 md:w-5 md:h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2 flex-shrink-0 cursor-pointer"
                disabled={loading}
              />
              <label htmlFor="rememberMe" className="ml-1.5 md:ml-2 text-xs md:text-base text-gray-700 cursor-pointer select-none">
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base py-2.5 md:py-3 mt-1"
            >
              {loading ? (
            ...
            <div className="text-center pt-1 md:pt-2">
              <Link href="/register" className="text-blue-600 hover:text-blue-700 text-xs md:text-base underline-offset-2 hover:underline">
